...
class Student:
    ...
    def report(self) :
        ...
        for topic in self.followed_topics():
            text += f"|{topic:^15}|{self.compute_average(topic):^15}|\n"
            text += LINE2
        return(text)
...
class Class:
    ...
    def get_student(self, f : str, l : str):
        key = (f, l)
    ...
    def load_grades_from_file(self, filename : str) :
        with open(filename,'r') as f : 
            for line in f :
                first, last, topic, *grades = line.strip().split(',')
    ...
    def catalog(self) :
        D = dict()
        for (key,std) in self.students.items() :
    ...
    def compute_averages(self) :
        D = dict()
        T = self.catalog()
        for (topic, nb) in T.items() :
            S = 0 
            for (keys, std) in self.students.items() :
                av = std.compute_average(topic)
                if av != -1 :
                    S += av
            D[topic] = S/nb
        return(D)
